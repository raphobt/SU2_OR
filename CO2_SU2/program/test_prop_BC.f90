...
!
     ...
     &           resnorm, guess_1, guess_2, T_span, v_span, e_span,helmho1,h
      REAL(pr):: p2, e2, cv2, cp2, s2, c2,helmho2,Helmholtz,eguess,out3,out2,press, Tguess
...
!     on calcule T a partir de P et rho
      CALL New_Rap1D(4, T, out2, resnorm, Niter,&
     &                             exitflag, p, Tguess, v, out3)
     ...
     &                             exitflag, p, eguess, T, out3)     ! ici peut etre
       CALL inter_energy(T,v,e2)
       CALL pressure(T,v,press)
      print*, 'v',v,'rho', 1.0/v
      print*, 'e ',e2
...
!
!
     CALL New_Rap2D(5, T, v, &
     &           resnorm, Niter, exitflag, h, s, guess_1, guess_2)
!     
      CALL pressure(T,v,p)
!
!      
