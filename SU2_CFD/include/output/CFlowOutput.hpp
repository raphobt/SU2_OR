...
/**********************/
   ...
   */
  su2double Get_h0(const CSolver* const*solver, unsigned long iPoint) const {
   ...
   */
  su2double Get_p0_T0(const CSolver* const*solver, unsigned long iPoint, int choice) const {
    ...
    if (Niter>=500 ){ cout << "Max iteration reached in SetTDState_hs in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
    if (T_out!=T_out){ cout << "NAN T in SetTDState_hs in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
    if (v_out!=v_out){ cout << "NAN v in SetTDState_hs in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
    if (v_out<=0.0){ cout << "Negative v in SetTDState_hs : v = " << v_out << " in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
    if (T_out<=100.0){ cout << "Too low T in SetTDState_hs : T = " << T_out << " in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
  
    __properties_MOD_inter_energy(&T_out, &v_out, &energy);
    if (energy!=energy){ cout << "NAN e in SetTDState_hs in CFlowOutput.hpp for p0, T0 surface-averaged values" << endl;
    }
    ...
    if (choice == 0){ return p0;
      ...
      cout << "Check the choice int value in CFlowOutput.cpp to output p0 and T0 field values..." << endl;
      return 0.0;
    }
