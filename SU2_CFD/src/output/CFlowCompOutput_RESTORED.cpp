...
void CFlowCompOutput::SetVolumeOutputFields(CConfig *config){
  ...
  AddVolumeOutput("CP", "Cp",             "PRIMITIVE", "Specific heat at constant pressure");
...
void CFlowCompOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  /* ---- */
    double pp, TT, cc, x_out, a_out, cp_out, vp_out, dummy;
    double Eref_SW=506779.92063833564;
    su2double vv = 1.0/Node_Flow->GetSolution(iPoint, 0);
    su2double p0 = Node_Flow->GetPressure(iPoint) + 0.5 / vv * Node_Flow->GetVelocity2(iPoint)/2.0;
    su2double ee;
    su2double hh;
    su2double h0;
    if (nDim == 3){
        ee = Node_Flow->GetSolution(iPoint, 4)*vv - Eref_SW - Node_Flow->GetVelocity2(iPoint)/2.0; // internal energy
        hh = ( Node_Flow->GetSolution(iPoint, 4) + Node_Flow->GetPressure(iPoint) )*vv - Node_Flow->GetVelocity2(iPoint)/2.0; 
        h0 = ( Node_Flow->GetSolution(iPoint, 4) + p0 )*vv; 
    } else {
        ee = Node_Flow->GetSolution(iPoint, 3)*vv - Eref_SW - Node_Flow->GetVelocity2(iPoint)/2.0;
        hh = ( Node_Flow->GetSolution(iPoint, 3) + Node_Flow->GetPressure(iPoint) )*vv - Node_Flow->GetVelocity2(iPoint)/2.0; 
        h0 = ( Node_Flow->GetSolution(iPoint, 4) + p0 )*vv; 
    }
    int flag;
 ...
 // SetVolumeOutputValue("TOTAL_PRESSURE", iPoint, Node_Flow->GetPressure(iPoint)+0.5*Node_Flow->GetSolution(iPoint, 0)*((Node_Flow->GetSolution(iPoint, 1)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 1)/Node_Flow->GetSolution(iPoint, 0))+(Node_Flow->GetSolution(iPoint, 2)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 2)/Node_Flow->GetSolution(iPoint, 0))+(Node_Flow->GetSolution(iPoint, 3)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 3)/Node_Flow->GetSolution(iPoint, 0))));
  
 // __transprop_MOD_cpco2(&cp_out, &vv, &vp_out, &x_out, &TT, &pp, &flag);
  
 //  SetVolumeOutputValue("CP", iPoint, cp_out);
  
 // SetVolumeOutputValue("TOTAL_TEMPERATURE", iPoint, Node_Flow->GetTemperature(iPoint)+0.5*((Node_Flow->GetSolution(iPoint, 1)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 1)/Node_Flow->GetSolution(iPoint, 0))+(Node_Flow->GetSolution(iPoint, 2)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 2)/Node_Flow->GetSolution(iPoint, 0))+(Node_Flow->GetSolution(iPoint, 3)/Node_Flow->GetSolution(iPoint, 0))*(Node_Flow->GetSolution(iPoint, 3)/Node_Flow->GetSolution(iPoint, 0)))/(cp_out+1e-10));

    SetVolumeOutputValue("TOTAL_PRESSURE", iPoint, p0);
    SetVolumeOutputValue("ENTHALPY", iPoint, hh);
    SetVolumeOutputValue("TOTAL_ENTHALPY", iPoint, h0);
